...
Combines all three report types into a single interactive menu:
  ...
  [2] Kentik       — parse alert logs → attack cycle CSV
  [3] Both         — run DefensePro + Kentik parsers together
  [4] Weekly HTML  — read kentik_attack_cycles_*.csv → weekly trend HTML report

Usage:
    python radware_reports.py                          # interactive menu
    python radware_reports.py --report dp              # skip menu, run DefensePro
    python radware_reports.py --report kentik          # skip menu, run Kentik
...
def dp_parse_logs(log_files: list) -> tuple:
    ...
    for log_file in log_files:
        with open(log_file, encoding="utf-8", errors="replace") as fh:
            for line in fh:
                ...
                m = re.search(
                    r"DFC00360.*for protected object (\S+)\. Criteria.*"
                    r"bandwidth (\S+) bps rate (\d+) pps", line)
                if m:
                    pps = int(m.group(3))
...
def run_kentik(log_dir: Path, out_file: Path, time_range: TimeRange) -> None:
    print()
    print("┌" + _separator("─", 48) + "┐")
...
# ══════════════════════════════════════════════════════════════════
#  Weekly HTML report  (reads kentik_attack_cycles_*.csv)
# ══════════════════════════════════════════════════════════════════
...
def run_weekly_report(time_range: TimeRange,
    ...
    if not csv_files:
        print("  ERROR: No source CSV files found. Run the Kentik parser first.")
        return
...
def main_menu() -> str:
    """Present the report selection menu.  Returns one of: dp, kentik, both, weekly."""
    print()
    print("╔" + _separator("═", 52) + "╗")
    ...
    print("    [2] Kentik       — parse logs → attack cycle CSV")
    print("    [3] Both         — run DefensePro + Kentik parsers")
    ...
    return {"1": "dp", "2": "kentik", "3": "both", "4": "weekly"}[choice]
...
def parse_args():
    ...
    p.add_argument("--report", choices=["dp", "kentik", "both", "weekly"],
...
def main():
    ...
    # ══════════════════════════════════════════════════════════════
    #  Parser reports (DefensePro / Kentik / Both)
    # ══════════════════════════════════════════════════════════════
    else:
        do_dp     = report_choice in ("dp",     "both")
        do_kentik = report_choice in ("kentik", "both")

        if do_dp:
            dp_log_dir  = _resolve(args.log_dir, "defensepro", "log_dir", "Input")
            dp_out_str  = (
                args.out if (do_dp and not do_kentik and args.out)
        ...
        if do_kentik:
            k_log_dir  = _resolve(args.log_dir, "kentik", "log_dir", "Input")
            k_out_str  = (
                args.out if (do_kentik and not do_dp and args.out)
